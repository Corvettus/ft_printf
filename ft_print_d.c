...
{
	if (tmp->precision_flag == 1 && tmp->width > 0 && tmp->precision <= tmp->width && tmp->flag != '0')
	{
...
//	if (tmp->flag == '+')
	...
	if ((tmp->flag == '+') && tmp->precision > tmp->width)
		tmp->arg_sign = 2;
	if (tmp->flag == '-' && tmp->flag2 == '+' && tmp->precision > tmp->width)
		tmp->precision--;
}

void	ft_if_flag_nll(var *tmp)
{
	if (tmp->precision > 0 && tmp->precision_flag == 1)
	{
		while ((int)ft_strlen(tmp->data) < tmp->precision)
			tmp->data = ft_strjoin("0", tmp->data);
		if (tmp->flag == '0' && tmp->arg_sign == -1)
			tmp->data = ft_strjoin("-", tmp->data);
		while (tmp->width > (int)ft_strlen(tmp->data))
			tmp->data = ft_strjoin(" ", tmp->data);
	}
	if (tmp->precision_flag == 0 && tmp->arg_sign == -1 && tmp->width > (int)ft_strlen(tmp->data))
		tmp->data = ft_strjoin("-", tmp->data);
}
...
char	*ft_print_d(var *tmp)
{
	...
	if ((int)ft_strlen(tmp->data) < tmp->precision && tmp->flag != '0' && tmp->precision_flag == 1)
		while (((int)ft_strlen(tmp->data) < tmp->precision && tmp->precision < tmp->width))
			tmp->data = ft_strjoin("0", tmp->data);
	if (tmp->arg_sign == -1 && tmp->flag != '0' && tmp->width > tmp->precision && tmp->precision_flag == 1)
		tmp->data = ft_strjoin("-", tmp->data);
	if (tmp->flag == '0')	
		ft_if_flag_nll(tmp);
